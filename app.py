...
def page_simulator():
    ...
    if not gemini_key:
        ...
        user_input = st.chat_input(
            "질문하세요. 예) 산미를 강하게 해줘 / 탄산가스 함량 기준은? / 증점제 필요한가?",
            key="gem_input"
        )
        if user_input:
            ...
            with st.spinner("🤖 Gemini 분석 중..."):
                try:
                    reply = _call_gemini_agent(
                        user_input, st.session_state.gemini_chat[:-1])
                    ...
                    st.session_state.gemini_chat.append(
                        {"role": "model", "text": f"❌ Gemini 오류: {e}"})
            st.rerun()

        if st.session_state.gemini_chat:
            if st.button("🔄 대화 초기화", key="gem_clear"):
