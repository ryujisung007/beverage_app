"""
🧪 음료개발 AI 플랫폼 v7.4 — radio 제거, selectbox 안정화, 정제수 수정
"""
...
ING_NAMES = ['(선택)', '✏️ 직접입력'] + ING_LIST   # selectbox 옵션
...
# ============================================================
# 헬퍼: selectbox 위젯 키 클리어 (rerun 전 호출)
# ============================================================
def clear_slot_widget_keys():
    """AI/가이드 배합비 로딩 후 rerun 전에 호출. selectbox 세션충돌 방지."""
    for i in range(20):
...
def load_formulation_with_estimation(formulation_list, auto_estimate=True):
    """AI/컨셉 배합비 → 슬롯 적용 + DB유사매칭 + 이화학 자동추정"""
...
def page_simulator():
    ...
    # AI 이화학 결과 테이블
    if st.session_state.ai_est_results:
        st.markdown('<div class="est-box">🤖 <b>AI 이화학분석 결과</b> — 배합표 Bx/산도/감미/단가에 자동 반영됨</div>', unsafe_allow_html=True)
        st.dataframe(pd.DataFrame(st.session_state.ai_est_results), use_container_width=True, hide_index=True)

    # [버튼2] 정제수 조정 — ing_total 기준으로 판단 (항상 표시)
    st.markdown("---")
    if ing_total > 100:
        st.error(f"⚠️ 원료합계 **{ing_total:.3f}%** > 100%. 원료 배합비를 줄이거나 정제수=0으로 조정하세요.")
        if st.button("💧 정제수 0%로 설정 (원료 배합비 초과)", type="primary", use_container_width=True):
            st.session_state.slots[19]['배합비(%)'] = 0
            st.rerun()
    elif ing_total < 100:
        st.info(f"원료합계 **{ing_total:.3f}%** — 정제수 **{water_pct:.3f}%** (합계 100%)")
        if abs(water_pct - safe_float(st.session_state.slots[19].get('배합비(%)', 0))) > 0.001:
            if st.button(f"💧 정제수 → {water_pct:.3f}% 조정 (합계 100%)", type="primary", use_container_width=True):
                st.session_state.slots[19]['배합비(%)'] = water_pct
                st.rerun()
    else:
        st.success(f"✅ 원료합계 {ing_total:.3f}% + 정제수 {water_pct:.3f}% = 합계 100.000%")
    ...
    with b2:
        ...
        if out_rows:
            st.download_button("📥 CSV", pd.DataFrame(out_rows).to_csv(index=False).encode('utf-8-sig'),
...
def page_reverse():
    ...
    if sel:
        ...
        if st.button("🔄 역설계 → 시뮬레이터", type="primary"):
            ...
            clear_slot_widget_keys(); st.success("✅")
...
def page_planner():
    ...
    with tabs[3]:
        ...
        if st.button("📝 보고서", type="primary"):
            ft = '\n'.join([f"{n}:{p:.3f}%" for n,p in active])
            with st.spinner("AI..."):
                r = call_gpt(OPENAI_KEY, persona, f"제품:{st.session_state.product_name}\n배합:\n{ft}\n종합 분석보고서")
                st.markdown(r)
